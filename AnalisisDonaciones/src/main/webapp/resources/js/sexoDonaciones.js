...
function drawChart() {  
  ...
  $('#tablaCompleta tr').each(function(index, element){
    if (index > 0 )
	 {				
        ...
        valorSexo = $(element).find("td").eq(7).text();
  ...
  data5.addColumn('string', 'Sexo');
  data5.addColumn('number', 'Valor');

  for(i = 0; i < arreglo.length; i++){	
    data5.addRow([ String( arreglo[i ] ), parseInt(arregloSexo[i ])]);
  }

  var options5 =
  {
    title: 'Grafico de donaciones: Sexo de los donantes',
    is3D : true,    
  ...
  new google.visualization.PieChart(document.getElementById('graficoSexo')).
   draw(data5, options5); 
  ...
  data.addColumn('number', 'Habitual Aceptado');
  ...
  var options = {
          title : 'Grafico de donaciones por Sexo',
 ...
 var options3 =
 {
	 title : 'Grafico de donaciones por Sexo (Porcentaje sobre las donaciones del mismo sexo)',
